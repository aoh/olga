...
; modifying some old code from a sample programming language assignment solution 
; to make a console sudoku. the old one seems to be lost, but was pretty terrible 
; anyway. it probably even used assignments.


; a solver for use in hints and puzzle generation

(define-module lib-sokoban
	...
	(define (remove-dups lst)
		(if (null? lst)
			null
			(cons (car lst)
				(remove-dups (rem (cdr lst) (car lst))))))

	(define (peers-of pos)
		(rem (sort < (remove-dups (foldr append null (areas-of pos)))) pos))
	...
	(define empty (put False 1 rasa))

	(define (play-boards str)
		(let loop ((input (string->bytes str)))
			(let ((input (read-solve-board empty 1 input)))
				(if input (loop input) 'done))))
					
	(show-state
		(solve (update empty 1 #b10000)))
...
; then a user interface, puzzle generator and presto. 
